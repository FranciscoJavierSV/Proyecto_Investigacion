...
// DB fetch duration (total DB fetch per request)
const dbFetchDuration = new client.Histogram({
	name: 'db_fetch_duration_ms',
	help: 'Duración total de fetch a la BD en ms',
	labelNames: ['api_type','operation'],
	buckets: [1,5,10,20,50,100,200,500,1000]
});

const httpResponsesTotal = new client.Counter({
	name: 'http_responses_total',
	help: 'Respuestas HTTP por codigo de estado',
...
const metricsMiddleware = (req, res, next) => {
	...
	httpActiveRequests.labels({ api_type: 'rest' }).inc();
	next();
}
...
register.registerMetric(dbFetchDuration);
